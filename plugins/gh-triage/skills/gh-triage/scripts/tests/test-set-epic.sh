...
# Case 1: usage error when epic-id is omitted
repo=$(setup_stub_project '{"items":[]}')
if (cd "$repo" && PATH="$repo/bin:$PATH" bash scripts/set-epic.sh PVTI_x 2>/dev/null); then
  fail "expected usage error when epic-id is omitted"
fi

# Case 2: sets the Epic Text field to the given epic id
repo=$(setup_stub_project '{"items":[]}')
(cd "$repo" && PATH="$repo/bin:$PATH" bash scripts/set-epic.sh PVTI_x ETYM-2)
